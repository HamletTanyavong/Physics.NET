namespace Physics.NET.Mathematics
{
    public static partial class Op
    {
            ...
            where T : class, ICoordinateSystem, I3D
        {
            ...
            {
                "Cartesian" => Subtract<T>((Vector3D<Cartesian>)a, (Vector3D<Cartesian>)b),
                "Cylindrical" => Subtract<T>((Vector3D<Cylindrical>)a, (Vector3D<Cylindrical>)b),
                "Spherical" => Subtract<T>((Vector3D<Spherical>)a, (Vector3D<Spherical>)b),
                _ => throw new TypeAccessException($"error: {coordinateSystem} is not a valid coordinate system"),
            };
        }

        public static Vector3D<T> Subtract<T>(Vector3D<Cartesian> a, Vector3D<Cartesian> b)
            where T : class, ICoordinateSystem, I3D
        {
            return new(a.X1 - b.X1, a.X2 - b.X2, a.X3 - b.X3);
        }

        public static Vector3D<T> Subtract<T>(Vector3D<Cylindrical> a, Vector3D<Cylindrical> b)
            where T : class, ICoordinateSystem, I3D
        {
        ...
        public static Vector3D<T> Subtract<T>(Vector3D<Spherical> a, Vector3D<Spherical> b)
            where T : class, ICoordinateSystem, I3D
        {
