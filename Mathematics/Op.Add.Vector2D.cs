namespace Physics.NET.Mathematics
{
    ...
    {
        public static Vector2D<T> Add<T>(Vector2D<T> a, Vector2D<T> b)
            where T : class, ICoordinateSystem, I2D
        {
            string coordinateSystem = typeof(T).Name;
            return coordinateSystem switch
            {
                "Cartesian" => Add<T>((Vector2D<Cartesian>)a, (Vector2D<Cartesian>)b),
                "Polar" => Add<T>((Vector2D<Polar>)a, (Vector2D<Polar>)b),
                _ => throw new TypeAccessException($"error: {coordinateSystem} is not a valid coordinate system"),
            };
        }

        public static Vector2D<T> Add<T>(Vector2D<Cartesian> a, Vector2D<Cartesian> b)
            where T : class, ICoordinateSystem, I2D
        {
            return new(a.X1 + b.X1, a.X2 + b.X2);
        }

        public static Vector2D<T> Add<T>(Vector2D<Polar> a, Vector2D<Polar> b)
            where T : class, ICoordinateSystem, I2D
        {
